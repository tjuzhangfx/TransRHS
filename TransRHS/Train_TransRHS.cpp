...
//
map<string,string> subRelation2relation;
map<string,double> relation2radius;
...
private:
    ...
    void gradient2(int e1_a,int e2_a,int rel_a,int flag)
    {

        for (int ii=0; ii<n; ii++)
        {

            double x = 2*(entity_vec[e1_a][ii] + relation_vec[rel_a][ii] - entity_vec[e2_a][ii]);
            if (L1_flag)
                if (x>0)
                    x=1;
                else
                    x=-1;
            relation_tmp[rel_a][ii] -= 	rate*x*flag;
            entity_tmp[e1_a][ii] 	-= 	rate*x*flag;
            entity_tmp[e2_a][ii]	-=	-1*rate*x*flag;
        }
    }
    void train_kb(int e1_a,int e2_a,int rel_a,int e1_b,int e2_b,int rel_b)
    {
		...
		string subRelation = id2relation[rel_a];
		while (subRelation2relation.count(subRelation) == 1)
		{
			
			string relation = subRelation2relation[subRelation];
			
			int relationId = relation2id[relation];
			int subRelationId = relation2id[subRelation];
			
			double radius1 = relation2radius[relation];
			double radius2 = relation2radius[subRelation];
			
			double d1 = calc_sum(e1_a,e2_a,relationId);
			double d2 = calc_sum(e1_a,e2_a,subRelationId);
			
			if(d1 > radius1)
			{
				res += d1 - radius1;
                gradient2(e1_a, e2_a,relationId, 1);
				relation2radius[relation] -= -rate;
			}
			
			if(d2 < radius1)
			{
				res += radius1 - d2;
				gradient2(e1_a, e2_a,subRelationId, -1);
				relation2radius[subRelation] -= rate;
			}
			...
			if(subRelation2relation.count(relation)==1)
			{
...
void prepare()
{
	...
	FILE* f3 = fopen("../data/subRelation2relation.txt","r");
	int x;
	...
	while (fscanf(f2,"%s%d",buf,&x)==2)
	{
		...
		id2relation[x]=st;
		relation_num++;
		relation2radius[st] = 1.5;
		
	}
	while (fscanf(f3,"%s%s",buf,buf1)==2)
	{
		string subRelation=buf;
		string relation=buf1;
		subRelation2relation[subRelation]=relation;
	}
